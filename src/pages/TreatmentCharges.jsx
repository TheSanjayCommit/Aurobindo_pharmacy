    ...
    <div className="charges-page">
      ...
      <section className="bg-gradient-to-br from-blue-50 via-white to-cyan-50 py-16">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              Treatment <span className="text-sky-600">Charges</span>
            </h1>
            <p className="text-xl text-gray-600 leading-relaxed mb-4">
              Transparent pricing for all dental treatments at affordable costs
            </p>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {affordableReasons.map((reason, index) => {
              const IconComponent = reason.icon;
              return (
                <div key={index} className="bg-gradient-to-br from-blue-50 to-white p-6 rounded-xl shadow-md border border-sky-100 text-center">
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4 max-w-6xl">
          <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">
            Individual <span className="text-sky-600">Treatments</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {treatments.map((treatment, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-sky-100 group"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="w-12 h-12 bg-sky-100 rounded-lg flex items-center justify-center mb-3 group-hover:bg-sky-600 transition-colors">
                        <IconComponent className="text-sky-600 group-hover:text-white transition-colors" size={24} />
                      </div>
                    ...
                    <Check className="text-sky-600 flex-shrink-0" size={24} />
                  </div>
                </div>
              );
            })}
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">
            Special <span className="text-sky-600">Packages</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {specialPackages.map((pkg, index) => (
              <div
                key={index}
                className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-sky-200 overflow-hidden"
              >
                <div className="h-48 overflow-hidden">
                  <img
                    src={pkg.image}
                ...
                <div className="p-6">
                  <h3 className="text-2xl font-bold text-gray-900 mb-2">{pkg.title}</h3>
                  <p className="text-gray-600 mb-4">{pkg.description}</p>
                  <div className="text-3xl font-bold text-sky-600 mb-6">{pkg.price}</div>
                  
                  <div className="space-y-3">
                    {pkg.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start space-x-3">
                        <Check className="text-sky-600 flex-shrink-0 mt-1" size={20} />
                        <span className="text-gray-700">{feature}</span>
                      </div>
                    ))}
                  </div>
      ...
      {/* Contact Section */}
      <section className="py-12 bg-gradient-to-r from-sky-500 to-cyan-500 text-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl md:text-3xl font-bold mb-4">
            For Details Please Contact:
          </h2>
